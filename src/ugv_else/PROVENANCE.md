...
| `explore_lite` (+ `explore_lite_msgs`) | [robo-friends/m-explore-ros2](https://github.com/robo-friends/m-explore-ros2) | `326cf8a` | Bumped 2026-06-25 from `e40e857` for the #48 marker-position fix (RViz sphere was drawn at `frontier.initial` not `frontier.centroid`); goal math in `frontier_search.cpp` is unchanged vs `e40e857`. Pin now also builds the in-repo `explore_lite_msgs` package (single `ExploreStatus` msg). The 3 tuned params (commits `6772fc5`, `94f95f2`) live in `ugv_nav/param/explore_lite.yaml`, launched via `ugv_nav/launch/explore.launch.py`. The repo's extra `map_merge` package is `COLCON_IGNORE`d by the build scripts. |
